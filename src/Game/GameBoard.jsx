      ...
      <div className="board">
        {
          ...
          cardsArray.map((item) => (
            <Card item={item} key={item.id} handleSelectedCards={handleSelectedCards} toggled={item === firstCard || item === secondCard || item.matched === true} stopflip={stopFlip} />
          ))
        }
      </div>

      {won !== 6 ? <div className="comments">Moves : {moves}</div> : <div className="comments">!!!!!! You Won in {moves} moves !!!!!!</div>}
